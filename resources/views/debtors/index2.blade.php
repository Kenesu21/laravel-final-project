        ...
        <table id="dataTable2" class="table table-striped table-bordered" style="width: 105%;">
            ...
            <tbody>
                @foreach ($lendings as $lending)
                    <tr>
                        <td>{{ $lending->Borrower_ID }}</td>
                        <td>{{ $lending->Borrower_Name }}</td>
                        <td>{{ $lending->Date_of_Transaction }}</td>
                        <td>{{ $lending->Amount_of_Money_Borrowed }}</td>
                        <td>{{ $lending->Remarks }}</td>
                        <td>
                            <form action="{{ route('pay.destroy', $lendings->Borrower_ID) }}" method="Post">
                                <a class="btn btn-info" href="{{ route('debtors.edit2', $lendings->Borrower_ID) }}"><i class="fa-sharp fa-solid fa-marker fa-bounce fa-2xs" style="color: #ffffff;"></i></a>
                                @csrf
                                @method('DELETE')
